...
string orderCommand(OrderBook &ob, vector<string> &tokens) {
	...
	bool isBuyOrder;
	if (tokens[2] == "buy") isBuyOrder = true;
	else if (tokens[2] == "sell") isBuyOrder = false;
	else return usageString;
...
string queryCommand(OrderBook &ob, vector<string> &tokens) {
	...
	if (tokens[1] == "level")
	{
		...
		if (tokens.size() != 4) {
			bool bid;
			if (tokens[2] == "bid") bid = true;
			else if (tokens[2] == "ask") bid = false;
			else return usageString;
...
string commandLine(OrderBook &ob, vector<string> &tokens) {
	...
	if (token == "order")
	{
		return orderCommand(ob, tokens);
	}
	if (token == "cancel")
	{
		return cancelCommand(ob, tokens);
	}
	if (token == "amend")
	{
		return amendCommand(ob, tokens);
	}
	if (token == "q")
	{
		return queryCommand(ob, tokens);
	}
